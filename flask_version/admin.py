...
def users():
    ...
    template_dict['number_of_users'] = len(app.notebook.valid_login_names()) if len(app.notebook.valid_login_names()) > 1 else None
    users = sorted(app.notebook.valid_login_names())
    del users[users.index('admin')]
    template_dict['users'] = [app.notebook.user(username) for username in users]
    template_dict['admin'] = app.notebook.user(g.username).is_admin()
...
def add_user():
    ...
    if 'username' in request.values:
        ...
        chara = string.letters + string.digits
        password = ''.join([choice(chara) for i in range(8)])
        if username in app.notebook.usernames():
            return render_template(os.path.join('html', 'settings', 'admin_add_user.html'),
                                   error='username_taken', username_input=username, **template_dict)
        app.notebook.add_user(username, password, '', force=True)
