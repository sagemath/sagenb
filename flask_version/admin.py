...
def users():
    ...
    template_dict['number_of_users'] = len(g.notebook.valid_login_names()) if len(g.notebook.valid_login_names()) > 1 else None
    users = sorted(g.notebook.valid_login_names())
    del users[users.index('admin')]
    template_dict['users'] = [g.notebook.user(username) for username in users]
    template_dict['admin'] = g.notebook.user(g.username).is_admin()
...
def add_user():
    ...
    if 'username' in request.values:
        ...
        chara = string.letters + string.digits
        password = ''.join([choice(chara) for i in range(8)])
        if username in g.notebook.usernames():
            return render_template(os.path.join('html', 'settings', 'admin_add_user.html'),
