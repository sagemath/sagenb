...
def home(username):
    if not g.notebook.user_manager().user_is_admin(g.username) and username != g.username:
        #XXX: i18n
        return current_app.message("User '%s' does not have permission to view the home page of '%s'."%(g.username, username), username=g.username)
    else:
...
def public_worksheet_cells(id, filename):
    ...
    try:
        ...
        return current_app.message("You do not have permission to access this worksheet", username=g.username) #XXX: i18n
...
def upload_worksheet():
    ...
    if url != '':
        ...
        if extension not in ["", ".txt", ".sws", ".zip", ".html", ".rst"]:
            ...
            return current_app.message("Unknown worksheet extension: %s. %s" % (extension, backlinks), username=g.username)
